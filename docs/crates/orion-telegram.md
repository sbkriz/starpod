...
orion_telegram::run_with_agent(agent, token).await?;

// With user allowlist
orion_telegram::run_with_agent_filtered(agent, token, allowed_users).await?;

// Send a notification (for cron job results)
orion_telegram::send_notification(&token, &user_ids, "Job completed").await;
```

...
orion_gateway::serve_with_agent(agent, config, notifier).await?;

// Build just the router (for testing or embedding)
let router = orion_gateway::build_router(state);
```
...
Optional API key auth via `ORION_API_KEY` environment variable:
